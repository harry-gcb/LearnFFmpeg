...
int main(int argc, char *argv[]) {
    ...
    in_fmt_ctx->streams[1]->discard = AVDISCARD_ALL;
        ...
        while (!eof) {
            ...
            if (!filter_graph) {
                ...
                av_bprintf(&args, "buffer=video_size=%dx%d:pix_fmt=%d:time_base=%d/%d:pixel_aspect=%d/%d:frame_rate=%d/%d[main];"
                                  ...
                                  "[result]buffersink",
                                  in_frame->width, in_frame->height, in_frame->format, tb.num, tb.den, sar.num, sar.den, fr.num, fr.den, in_frame->width/2, in_frame->height/2);
                ...
                result_ctx = avfilter_graph_get_filter(filter_graph, "Parsed_buffersink_2");
            }
            if (!eof) {
