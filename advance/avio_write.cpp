...
#define OUTPUT_BUF_SIZE (1024*1024*10)
...
static int write_packet(void *opaque, uint8_t *buf, int buf_size) {
    Buffer *buffer = (Buffer *)opaque;
    memcpy(buffer->ptr, buf, buf_size);
    buffer->ptr += buf_size;
    buffer->size += buf_size;
    return buf_size;
}
...
int main(int argc, char *argv[]) {
    ...
    Buffer input = { 0 };
    size_t length = 0;
    uint8_t *data = read_whole_file(argv[1], &length);
    input.ptr = data;
    input.ori_ptr = data;
    input.size = length;
    input.ori_size = length;
    uint8_t *in_avio_ctx_buffer = nullptr;
    int avio_ctx_buffer_size = AVIO_CTX_BUFFER_SIZE;
    in_avio_ctx_buffer = (uint8_t *)av_malloc(avio_ctx_buffer_size);
    if (!in_avio_ctx_buffer) {
        printf("av_malloc failed: %d\n", ret);
        return ret;
    }

    AVIOContext *in_avio_ctx = nullptr;
    in_avio_ctx = avio_alloc_context(in_avio_ctx_buffer, avio_ctx_buffer_size, 0, &input, &read_packet, nullptr, &seek_packet);
    if (!in_avio_ctx) {
        printf("avio_alloc_context failed\n");
        return 0;
    }

    in_fmt_ctx->pb = in_avio_ctx;
    ...
    Buffer output = { 0 };
    output.ptr = (uint8_t*)av_malloc(OUTPUT_BUF_SIZE);
    output.ori_ptr = output.ptr;
    output.size = 0;
    output.ori_size = OUTPUT_BUF_SIZE;
    uint8_t *out_avio_ctx_buffer = (uint8_t *)av_malloc(avio_ctx_buffer_size);
    if (!out_avio_ctx_buffer) {
        printf("av_malloc failed\n");
        return ret;
    }
    AVIOContext *out_avio_ctx = avio_alloc_context(out_avio_ctx_buffer, avio_ctx_buffer_size, 1, (void *)(&output), nullptr, &write_packet, nullptr);
    if (!out_avio_ctx) {
        printf("avio_alloc_context failed\n");
        return ret;
    }
    ...
    ret = avformat_alloc_output_context2(&out_fmt_ctx, nullptr, "flv", nullptr);
    if (ret != 0) {
    ...
    av_free(in_avio_ctx_buffer);
    avio_context_free(&in_avio_ctx);

    av_free(out_avio_ctx_buffer);
    avio_context_free(&out_avio_ctx);
    ...
    fwrite(output.ori_ptr, 1, output.size, fp);
    fclose(fp);

    av_free(input.ori_ptr);
    av_free(output.ori_ptr);
