    ...
    while (!end) {
        ...
        while (!end) {
            ...
            if (!filter_graph) {
                ...
                av_bprintf(&scale_args,"[0:v]scale=%d:%d",in_frame->width/2,  in_frame->height/2);
                ret = avfilter_graph_parse2(filter_graph, scale_args.str, &inputs, &outputs);
                if (ret != 0) {
                    printf("avfilter_graph_parse2 failed: %d\n", ret);
                    return ret;
                }

                for (current = inputs; current; current = current->next) {
                    ret = avfilter_link(main_ctx, 0, current->filter_ctx, 0);
                    if (ret != 0) {
                        printf("avfilter_link failed: %d\n", ret);
                        return ret;
                    }
                }
                for (current = outputs; current; current = current->next) {
                    ret = avfilter_link(current->filter_ctx, 0, result_ctx, 0);
                    if (ret != 0) {
                        printf("avfilter_link failed: %d\n", ret);
                        return ret;
                    }
