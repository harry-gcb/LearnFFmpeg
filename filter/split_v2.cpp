        ...
        while (!eof) {
            ...
            if (!filter_graph) {
                ...
                av_bprintf(&args, "buffer=video_size=%dx%d:pix_fmt=%d:time_base=%d/%d:pixel_aspect=%d/%d:frame_rate=%d/%d[main];"
                                  ...
                                  "[result]buffersink",
                                  in_frame->width, in_frame->height, in_frame->format, tb.num, tb.den, sar.num, sar.den, fr.num, fr.den, in_frame->width/2, in_frame->height/2, in_frame->width/4*3, in_frame->height/4*3);
